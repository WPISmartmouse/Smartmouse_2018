...
\begin{figure}[h]
  \begin{tikzpicture} [scale=2]
    ...
    \draw (-1.2, 2.1) node {$row$};
    \draw (0.1, 3.1) node {$col$};
    ...
    \draw [rotate around={15:(0,0)}] (4, -.3) node {$(c,r,\theta)$};
...
\begin{align}
  ...
  r &\leftarrow r + R\Big(-\sin{(\frac{v_r-v_l}{W}\Delta t)}(\sin{\theta})+\cos{(\frac{v_r-v_l}{W}\Delta t)}(\cos{\theta}\Big) - R\cos{\theta} \\
  \shortintertext{let $\frac{v_r-v_l}{W}=a$, and $\theta=b$, we can use $-\sin{a}\sin{b}+\cos{a}\cos{b}=\cos{(a+b)}$}
  r &\leftarrow r + R\Bigg(\cos{\Big(\frac{v_r-v_l}{W}\Delta t + \theta\Big)}\Bigg) - R\cos{\theta} \\
