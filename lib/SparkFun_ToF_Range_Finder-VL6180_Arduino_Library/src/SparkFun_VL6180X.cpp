...
void VL6180x::VL6180xDefautSettings(void){
  ...
  VL6180x_setRegister(VL6180X_SYSRANGE_RANGE_CHECK_ENABLES, 0x10); // disable early convergence
...
void VL6180x::VL6180xMouseSettings(void){
  //Recommended settings from datasheet
  //http://www.st.com/st-web-ui/static/active/en/resource/technical/document/application_note/DM00122600.pdf
  ...
  VL6180x_setRegister(VL6180X_SYSTEM_MODE_GPIO1, 0x00); // Set GPIO1 high when sample complete
  VL6180x_setRegister(VL6180X_READOUT_AVERAGING_SAMPLE_PERIOD, 0x30); //Set Avg sample period
  VL6180x_setRegister(VL6180X_SYSALS_ANALOGUE_GAIN, 0x46); // Set the ALS gain
  VL6180x_setRegister(VL6180X_SYSRANGE_VHV_REPEAT_RATE, 0xff); // Set auto calibration period (Max = 255)/(OFF = 0)
  VL6180x_setRegister(VL6180X_SYSALS_INTEGRATION_PERIOD, 0x63); // Set ALS integration time to 100ms
  ...
  //Optional settings from datasheet
  //http://www.st.com/st-web-ui/static/active/en/resource/technical/document/application_note/DM00122600.pdf
  VL6180x_setRegister(VL6180X_SYSRANGE_INTERMEASUREMENT_PERIOD, 10); // TODO tune this.
  VL6180x_setRegister(VL6180X_SYSTEM_INTERRUPT_CONFIG_GPIO, 0x00); //disable all
  //Additional settings defaults from community
  VL6180x_setRegister(VL6180X_SYSRANGE_MAX_CONVERGENCE_TIME, 0x10); // TODO tune this.
  VL6180x_setRegister(VL6180X_SYSRANGE_RANGE_CHECK_ENABLES, 0x10); // disable early convergence
  // VL6180x_setRegister16bit(VL6180X_SYSRANGE_EARLY_CONVERGENCE_ESTIMATE, 0x7B );
  // VL6180x_setRegister16bit(VL6180X_SYSALS_INTEGRATION_PERIOD, 0x64);

  // VL6180x_setRegister(VL6180X_READOUT_AVERAGING_SAMPLE_PERIOD,0x48);
  // VL6180x_setRegister(VL6180X_SYSALS_ANALOGUE_GAIN,0x40);
  // VL6180x_setRegister(VL6180X_FIRMWARE_RESULT_SCALER,0x01);
}
...
uint8_t VL6180x::setContinuousRange(){
  getDistance(); // switch out of continuous range if already in
  VL6180x_setRegister(VL6180X_SYSRANGE_START, 0x03); 

}

uint8_t VL6180x::getDistanceContinuous(){
  uint8_t dist = VL6180x_getRegister(VL6180X_RESULT_RANGE_VAL);
  // VL6180x_setRegister(VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x07);
  return dist;
}

uint8_t VL6180x::getResultRangeStatus(){
  uint8_t dist = VL6180x_getRegister(VL6180X_RESULT_RANGE_STATUS);
  return dist;
}
